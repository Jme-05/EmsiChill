...
/** Ubicación serializable utilizada por /back y los puntos anteriores a un teletransporte. */
public record SavedLocation(String world, double x, double y, double z, float yaw, float pitch) {
    public static SavedLocation from(final Location location) {
        return new SavedLocation(location.getWorld().getName(), location.getX(), location.getY(), location.getZ(),
    ...
    public static SavedLocation load(final YamlConfiguration yaml, final String path) {
        String world = yaml.getString(path + ".world");
        if (world == null) {
            return null;
        }
        return new SavedLocation(world, yaml.getDouble(path + ".x"), yaml.getDouble(path + ".y"),
